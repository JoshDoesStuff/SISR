...
SISR includes support to emulate DualShock4 controllers instead of Xbox360 controllers.  
...
!!! info "Gyro Passthrough"
    ...
    - Steam Controller (1/2) / Deck specific:  
        Gyro data is not transmitted from Steam to SISR unless gyro is bound to something **other than "_None_"** in Steam Input configuration.  
...
Enabling DS4 emulation in SISR is done via CLI-argument / configuration file
    ...
    Pass `--ct=dualshock4` as launch argument to SISR.  
